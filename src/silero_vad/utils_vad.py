...
class OnnxWrapper():
    ...
    def __call__(self, x, sr: int):
        ...
        if sr in [8000, 16000]:
            ort_inputs = {'input': x.numpy(), 'state': self._state.numpy(),
...
@torch.no_grad()
def get_vad_probs(audio: torch.Tensor,
                  model,
                  sampling_rate: int = 16000,
                  progress_tracking_callback: Callable[[float], None] = None,
                  window_size_samples: int = 512,
                  return_audio_length_samples: bool = False
                  ) -> List[float]:
    ...
    if return_audio_length_samples:
        return speech_probs, audio_length_samples

    return speech_probs


def get_timestamps_from_probs(speech_probs: List[float],
    ...
    min_silence_samples_at_max_speech = sampling_rate * \
        min_silence_at_max_speech / 1000
    ...
    for i, speech_prob in enumerate(speech_probs):
        ...
        if triggered and (cur_sample - current_speech['start'] > max_speech_samples):
            if use_max_poss_sil_at_max_speech and possible_ends:
                # use the longest possible silence segment in the current speech chunk
    ...
    for i, speech in enumerate(speeches):
        ...
        if i != len(speeches) - 1:
            ...
            if silence_duration < 2 * speech_pad_samples:
                ...
                speeches[i+1]['start'] = int(max(0, speeches[i+1]
                                             ['start'] - silence_duration // 2))
            else:
                speech['end'] = int(
                    min(audio_length_samples, speech['end'] + speech_pad_samples))
                speeches[i+1]['start'] = int(max(0, speeches[i+1]
                                             ['start'] - speech_pad_samples))
        else:
            speech['end'] = int(
                min(audio_length_samples, speech['end'] + speech_pad_samples))
...
def get_speech_timestamps(audio: torch.Tensor,
                          ...
                          time_resolution: int = 1,
                          visualize_probs: bool = False,
                          progress_tracking_callback: Callable[[
                              float], None] = None,
                          neg_threshold: float = None,
                          window_size_samples: int = 512,
                          min_silence_at_max_speech: int = 98,
    ...
    """
    speech_probs, audio_length_samples = get_vad_probs(
        audio, model, sampling_rate, progress_tracking_callback,
        return_audio_length_samples=True)
    speeches = get_timestamps_from_probs(
        speech_probs, audio_length_samples=audio_length_samples,
...
class VADIterator:
    ...
    def __call__(self, x, return_seconds=False, time_resolution: int = 1):
        ...
        if (speech_prob >= self.threshold) and not self.triggered:
            self.triggered = True
            speech_start = max(0, self.current_sample -
